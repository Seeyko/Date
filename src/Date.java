public class Date {
	 ...
	 * @return this (my)date
	 */
	public Date nextDay() { //pas fini
		Date d = this;		
		if(maxDayOfMonth(d.getMonth(), d.getYear())==d.getDay()){
			if(d.getMonth() == 12){
				d = new Date(1, 1, d.getYear()+1);
			}else{
				d = new Date(1, d.getMonth()+1, d.getYear());
			}
		}else{
			d = new Date(d.getDay() + 1, d.getMonth(), d.getYear());
		}
		return d;
	}
	...
	private static boolean DateNextDayTest(int day, int month, int year){
		Date d = new Date(day, month, year);
		Date tommorowDate = d.nextDay();
		
		
		if(d.getDay() == maxDayOfMonth(d.getMonth(), d.getYear()))
		{
			if(d.getMonth() == 12){
				
				if(tommorowDate.getMonth() != 1 && tommorowDate.getDay() != 1)
				{
					return false;
				}
				
			}else{
				if(tommorowDate.getDay() != 1)
				{
					return false;
				}
			}
